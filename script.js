...
function loadStudentData() {
    try {
        if (processedData) return Promise.resolve(true); // Return cached data if available
        
        processedData = IsakhaData.processedData;
        if (processedData && Object.keys(processedData).length > 0) {
            console.log('Data loaded successfully');
            console.log('Available Student IDs:', Object.keys(processedData));
            return Promise.resolve(true);
        }
        return Promise.reject(new Error('No student data available'));
    } catch (error) {
